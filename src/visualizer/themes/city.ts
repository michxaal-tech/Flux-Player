...
export const CITY: ThemeDraw = ({ c, w, h, freq, liveAudio, vt, beat, beatE, bassV, I, TK, C1, C2, CMix, L }) => {
  ...
  if (beat) {
    for (let k = 0; k < 6; k++) {
      ...
      L.cityH[i] = Math.min(1.2, 0.7 + Math.random() * 0.5);
    }
  }
  const baseY = h * 0.8;
  const bw2 = w / N;
  for (let i = 0; i < N; i++) {
    ...
    // windows — flare on the beat
    c.fillStyle = C1(0.5 + beatE * 0.5, 78 + beatE * 12);
    ...
    // reflection
    c.fillStyle = CMix(i / N, 0.14 + beatE * 0.1, 45);
    c.fillRect(i * bw2 + 2, baseY + 2, bw2 - 4, bh * 0.35);
  }
  c.fillStyle = C2(0.8 + beatE * 0.2);
  c.fillRect(0, baseY, w, (2 + beatE * 3) * TK);
};
